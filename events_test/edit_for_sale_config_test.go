...
func TestEditForSaleConfigMakeForSale(t *testing.T) {
	...
	err = client.Events.EditForSaleConfig(test_util.RequestContext(), event.Key, []events.ObjectAndQuantity{{Object: "A-1"}, {Object: "A-2"}}, nil)
	require.NoError(t, err)
...
func TestEditForSaleConfigMakeNotForSale(t *testing.T) {
	...
	err = client.Events.EditForSaleConfig(test_util.RequestContext(), event.Key, nil, []events.ObjectAndQuantity{{Object: "A-1"}, {Object: "A-2"}})
	require.NoError(t, err)
...
func TestEditForSaleConfigMakeAreaPlacesNotForSale(t *testing.T) {
	...
	err = client.Events.EditForSaleConfig(test_util.RequestContext(), event.Key, nil, []events.ObjectAndQuantity{{Object: "GA1", Quantity: 5}})
	require.NoError(t, err)
